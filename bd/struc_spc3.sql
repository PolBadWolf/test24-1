...
CREATE TABLE `datas`  (
  ...
  `unclenchingTime` float NULL DEFAULT NULL,
  `clenchingTime` float NULL DEFAULT NULL,
...
CREATE TABLE `pusherstype_logger`  (
  ...
  `unclenchingTime` float NULL DEFAULT NULL COMMENT 'номинальное время разжатия',
  `clenchingTime` float NULL DEFAULT NULL COMMENT 'номинальное время зжатия',
